...
from lab3.db import Base, get_session  # noqa: E402
from lab3.main import app  # noqa: E402
...
def test_get_all_books_basic_with_cursor_pagination(client: TestClient):
    ...
    response2 = client.get("/books", params={"limit": 2, "cursor": cursor})
    assert response2.status_code == 200
    data2 = response2.json()
    assert data2["limit"] == 2
    assert len(data2["items"]) == 2

    # Третя сторінка (остання)
    cursor2 = data2["next_cursor"]
    if cursor2 is not None:
        response3 = client.get("/books", params={"limit": 2, "cursor": cursor2})
        assert response3.status_code == 200
        data3 = response3.json()
        # на останній сторінці next_cursor має бути None
